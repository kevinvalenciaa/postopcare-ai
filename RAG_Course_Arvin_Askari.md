...
2) Embedding: Apply an embedding model to the Library, store vectors, and start enabling natural language queries. 

3) Chunking: The process of splitting large documents into smaller pieces for better retrieval accuracy.  

4) Retrieval: The step where the system finds the most relevant chunks based on vector similarity.  

5) Context Window: The portion of text the LLM can “see” when generating an answer.
